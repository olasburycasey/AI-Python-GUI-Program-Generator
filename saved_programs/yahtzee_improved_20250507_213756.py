...
class YahtzeeGUI:
    def __init__(self, master):
        ...
        master.geometry("600x800")  # Adjust window size
        ...
        self.yahtzee_bonus_count = 0 # Number of yahtzee scored
        self.yahtzee_scored = False


        # GUI elements
        self.label_dice = [tk.Label(master, text="0", relief="solid", borderwidth=1, width=3, font=("Arial", 24)) for _ in range(5)]  # Text labels
        self.button_roll = tk.Button(master, text="Roll Dice", command=self.roll_dice, font=("Arial", 12))
        self.button_hold = [tk.Button(master, text="Hold", command=lambda i=i: self.toggle_hold(i), font=("Arial", 10)) for i in range(5)]
        self.label_rolls_remaining = tk.Label(master, text="Rolls Remaining: 3", font=("Arial", 12))
        self.label_scorecard = tk.Label(master, text="Scorecard:", font=("Arial", 14))
        self.text_scorecard = tk.Text(master, height=15, width=40, state="disabled", font=("Arial", 10)) # make read-only

        # Layout with Padding
        for i in range(5):
            self.label_dice[i].grid(row=0, column=i, padx=5, pady=5)
            self.button_hold[i].grid(row=1, column=i, padx=5, pady=5)

        self.button_roll.grid(row=2, column=0, columnspan=5, padx=5, pady=10)
        self.label_rolls_remaining.grid(row=3, column=0, columnspan=5, padx=5, pady=5)
        self.label_scorecard.grid(row=4, column=0, columnspan=5, padx=5, pady=5)
        self.text_scorecard.grid(row=5, column=0, columnspan=5, padx=5, pady=5)
        ...
        for i, cat in enumerate(self.categories):
           self.category_buttons[cat] = tk.Button(master, text=cat, command=lambda c=cat: self.score_category(c), state="disabled", font=("Arial", 10))
           self.category_buttons[cat].grid(row=6 + i, column=0, columnspan=5, padx=5, pady=2)  # Place buttons below scorecard


        self.label_total_score = tk.Label(master, text="Total Score: 0", font=("Arial", 14))
        self.label_total_score.grid(row=6 + len(self.categories), column=0, columnspan=5, padx=5, pady=10)  # Place total score label
    ...
    def update_dice_labels(self):
        """Updates the dice labels with either text or images."""
        for i in range(5):
           self.label_dice[i].config(text=str(self.dice[i]))

    def update_hold_button_states(self):
      """Resets the hold buttons to their initial state."""
      for i in range(5):
    ...
    def score_category(self, category):
        ...
        if category in ["Aces", "Twos", "Threes", "Fours", "Fives", "Sixes"]:
            self.upper_section_score += score
        ...
        if len(self.scorecard) == 6 and self.upper_section_score >= 63:
            messagebox.showinfo("Yahtzee", "Upper Section Bonus! +35 points!")
            self.total_score += 35
            self.label_total_score.config(text=f"Total Score: {self.total_score}")
